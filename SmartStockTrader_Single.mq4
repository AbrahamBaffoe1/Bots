...
// PHASE 3A: MARKET STRUCTURE DETECTION - Check for higher highs/lows (bullish) or lower highs/lows (bearish)
bool CheckMarketStructure(string symbol, bool isBuySignal) {
   int lookback = 20;  // Check last 20 H1 bars for structure
   ...
   for(int i = 2; i < lookback; i++) {
      ...
      double nextLow = iLow(symbol, PERIOD_H1, i-1);
   ...
   if(ArraySize(swingHighs) < 2 || ArraySize(swingLows) < 2) {
      if(VerboseLogging) Print("⚠ ", symbol, " - Insufficient swing points for structure analysis");
      return true;  // Skip if not enough data
   }

   // For BUY: Check for higher highs and higher lows (uptrend structure)
   if(isBuySignal) {
      bool higherHighs = (swingHighs[0] > swingHighs[1]);  // Most recent > previous
      bool higherLows = (swingLows[0] > swingLows[1]);

      if(higherHighs && higherLows) {
         if(VerboseLogging) Print("✓ ", symbol, " - Bullish structure: Higher highs & higher lows");
         return true;
      } else {
         if(VerboseLogging) Print("✗ ", symbol, " - No bullish structure (HH: ", higherHighs, ", HL: ", higherLows, ")");
         return false;
      }
   }

   // For SELL: Check for lower highs and lower lows (downtrend structure)
   bool lowerHighs = (swingHighs[0] < swingHighs[1]);
   bool lowerLows = (swingLows[0] < swingLows[1]);

   if(lowerHighs && lowerLows) {
      if(VerboseLogging) Print("✓ ", symbol, " - Bearish structure: Lower highs & lower lows");
      return true;
   } else {
      if(VerboseLogging) Print("✗ ", symbol, " - No bearish structure (LH: ", lowerHighs, ", LL: ", lowerLows, ")");
      return false;
   }
}

// PHASE 3A: SUPPORT/RESISTANCE & ROOM-TO-TARGET CHECK - Ensure min 3x ATR room to target
bool CheckRoomToTarget(string symbol, bool isBuySignal) {
   double atr = iATR(symbol, PERIOD_H1, 14, 0);
   double currentPrice = isBuySignal ? MarketInfo(symbol, MODE_ASK) : MarketInfo(symbol, MODE_BID);

   // Find nearest resistance (for BUY) or support (for SELL) in last 100 bars
   int lookback = 100;
   double level = 0;

   if(isBuySignal) {
      // Find nearest resistance above current price
      double resistance = 999999;
      for(int i = 1; i < lookback; i++) {
         double high = iHigh(symbol, PERIOD_H1, i);
         if(high > currentPrice && high < resistance) {
            resistance = high;
         }
      }
      ...
      if(resistance < 999999) {
         ...
         double atrMultiple = distance / atr;

         if(atrMultiple < 3.0) {
            if(VerboseLogging) Print("✗ ", symbol, " - Resistance too close (", DoubleToString(atrMultiple, 1), " ATR, need 3.0 ATR)");
            return false;
         }

         if(VerboseLogging) Print("✓ ", symbol, " - Room to resistance: ", DoubleToString(atrMultiple, 1), " ATR");
         return true;
      }

      // No resistance found = plenty of room
      if(VerboseLogging) Print("✓ ", symbol, " - No resistance nearby, plenty of room");
      return true;
   } else {
      ...
      for(int i = 1; i < lookback; i++) {
         double low = iLow(symbol, PERIOD_H1, i);
         if(low < currentPrice && low > support) {
            support = low;
         }
      }
      ...
      if(support > 0) {
         ...
         if(atrMultiple < 3.0) {
            if(VerboseLogging) Print("✗ ", symbol, " - Support too close (", DoubleToString(atrMultiple, 1), " ATR, need 3.0 ATR)");
            return false;
         }

         if(VerboseLogging) Print("✓ ", symbol, " - Room to support: ", DoubleToString(atrMultiple, 1), " ATR");
         return true;
      }

      // No support found = plenty of room
      if(VerboseLogging) Print("✓ ", symbol, " - No support nearby, plenty of room");
      return true;
   }
}
